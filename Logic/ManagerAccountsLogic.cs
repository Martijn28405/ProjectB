...
class ManagerAccountsLogic
{
    private JsonAccessor<ManagerAccountModel> _accessor;
    private List<ManagerAccountModel> _accounts;

    //Static properties are shared across all instances of the class
    //This can be used to get the current logged in account from anywhere in the program
    //private set, so this can only be set by the class itself
    static public ManagerAccountModel? CurrentAccount { get; private set; }

    public ManagerAccountsLogic()
    {
        _accessor = new JsonAccessor<ManagerAccountModel>(@"DataSources/managers.json");
        _accounts = _accessor.LoadAll();
    }

    public void CreateAccount()
    {
        ManagerAccountsLogic redoLogin = new ManagerAccountsLogic();
        ...
        {
            ManagerAccountModel acc = new ManagerAccountModel(emailAddress, password, fullName);
            UpdateList(acc);
            Console.WriteLine("Your account has been succesfully created!");
        }
    }

    public void UpdateList(ManagerAccountModel acc)
    {
    ...
    public ManagerAccountModel GetById(int id)
    {
        return _accounts.Find(account => account.Id == id);
    }

    public ManagerAccountModel CheckLogin(string email, string password)
    {
